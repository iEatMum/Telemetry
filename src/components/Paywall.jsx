...
import { PLANS, getLocalizedPlans, hasLapsed, purchase, restore, useEntitlement } from '../lib/purchases.js'
  ...
  return (
    <div className="relative overflow-hidden border border-line bg-surface2">
      ...
      <div className="relative flex flex-col items-center px-4 py-5 text-center">
        <span
          aria-hidden
          className="flex h-9 w-9 items-center justify-center rounded-full border border-accent text-[12px] text-accent"
        >
          ◆
        </span>
        <span className="mt-2 font-clock text-[10px] uppercase tracking-widest2 text-muted">Coach · sealed</span>
        <p className="mt-1.5 max-w-[15rem] font-serif text-[14px] italic leading-relaxed text-muted">{line}</p>
        <button
          type="button"
          onClick={openPaywall}
          className="mt-3 rounded-control bg-accent px-4 py-2.5 font-clock text-[11px] font-semibold uppercase tracking-widest2 text-accent-ink"
        >
          Open the coach’s page · {TRIAL_DAYS}-day free
        </button>
        <span className="mt-2 font-clock text-[10px] uppercase tracking-wide text-faint">
          ...
          <>
            <div className="pt-10">
              ...
              {lapsed ? (
                ...
                <>
                  ...
                  {personal && (
                    <p className="mt-2 border-l-2 border-accent-deep pl-3 text-[13px] leading-relaxed text-ink">
                      {personal}
                    </p>
                  )}
                </>
            ...
            <div className="mt-5 grid grid-cols-2 border-t border-line">
              <div className="border-r border-line py-4 pr-3">
                ...
                <ul className="mt-2.5 space-y-2 text-[12px] leading-snug text-muted">
              ...
              <div className="py-4 pl-3">
                <div className="font-clock text-[10px] uppercase tracking-widest2 text-accent">
            ...
            <div className="mt-6 space-y-2.5">
              {plans.map((p) => {
                ...
                return (
                  ...
                  >
                    <span>
                      <span className={`block text-[15px] ${on ? 'font-medium' : ''} text-ink`}>
                        {p.line}
                      </span>
                      <span className="block text-[12px] text-muted">{p.sub}</span>
                    </span>
                    <span className="font-clock tnum text-[17px] text-ink">
                      {p.price}
                      <span className="text-[11px] text-muted"> / {p.per}</span>
                    </span>
                  </button>
                )
              })}
            ...
            <div className="mt-8 space-y-3">
              ...
              <div className="flex flex-wrap items-center justify-center text-[12px] text-muted">
                <button
                  type="button"
                  onClick={doRestore}
