...
function addMonths(date, n) {
  // Clamp the day so we never overflow into the next month. Native setMonth
  // turns Jan 31 + 1mo into Mar 3 (skipping February) and then permanently
  // drifts the cadence. Snap to the target month's last valid day instead.
  const d = new Date(date)
  const day = d.getDate()
  d.setDate(1)
